...
C2FCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<safeinsure_address\> \<monthly_payment_safeinsure\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ C2FCoin, _12_ times over the course of a year totaling _24000_ C2FCoin.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<safeinsure_address\> \<monthly_payment_safeinsure\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` C2FCoin.
 ...
 * vote-many          - Vote on a C2FCoin initiative
 * vote-alias         - Vote on a C2FCoin initiative
 * vote               - Vote on a C2FCoin initiative/budget
